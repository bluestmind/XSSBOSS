...
class BurpService:
    ...
    def import_from_rest(
        ...
        Args:
            db: Database session
            target_id: Target ID
            api_url: Burp REST API Base URL (e.g. http://127.0.0.1:1337)
            api_key: Burp REST API key (optional)
            task_id: Burp scan task ID returned by POST /scan
            
        Returns:
            Status summary of the REST import
        """
        headers = {}
        if api_key:
