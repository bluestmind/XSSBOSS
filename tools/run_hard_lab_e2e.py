...
def _looks_like_candidate(case: dict, payload: str, token: str) -> bool:
    ...
    import html
    lowered = html.unescape(checked).lower()
...
def _set_env(database_url: str | None, oracle_url: str):
    os.environ["DATABASE_URL"] = database_url or f"sqlite:///{(ROOT / 'xssboss.db').as_posix()}"
    os.environ["CELERY_TASK_ALWAYS_EAGER"] = "True"
    os.environ["ORACLE_SERVER_URL"] = oracle_url
...
def run_e2e(
    ...
    _set_env(database_url, oracle_url)
    ...
    seed_hard_lab(base_url=base_url, database_url=database_url, reset=reset, create_experiments=False)
    ...
    try:
        ...
        experiment = Experiment(
            target_id=target.id,
            name=f"Hard Lab - Browser Proof {datetime.now(UTC).isoformat(timespec='seconds')}",
        ...
        for case in manifest["cases"]:
            if case_slugs and case["slug"] not in case_slugs:
                continue
            ...
            if not candidates:
                ...
                case_results.append({"slug": case["slug"], "detected": False, "error": "no_payload"})
            ...
            if not endpoint:
                ...
                case_results.append({"slug": case["slug"], "detected": False, "error": "missing_seeded_endpoint"})
            ...
            if not param:
                ...
                case_results.append({"slug": case["slug"], "detected": False, "error": "missing_seeded_param"})
        ...
        # open. Dispose the acceptance engine before its temporary directory is
        # removed so a successful proof exits successfully as well.
        try:
            from backend_api.db.base import engine
    ...
    evaluated_truth = [
        ...
        if (include_controls or case["expected_vulnerable"])
        and (not case_slugs or case["slug"] in case_slugs)
    ]
    metrics = evaluate_accuracy(evaluated_truth, case_results)
    if artifact_path:
...
def main() -> int:
    ...
    parser.add_argument(
        "--artifact",
        type=Path,
        default=ROOT / "reports" / "hard_lab_accuracy.json",
        help="Write machine-readable precision/recall evidence",
    )
    parser.add_argument("--case", action="append", dest="cases", help="Run only one manifest slug; repeatable")
    parser.add_argument(
        "--database-url",
        help="Optional acceptance-test database. Defaults to a disposable SQLite database.",
    )
    args = parser.parse_args()

    def execute(database_url: str) -> int:
        return run_e2e(
            ...
            case_slugs=set(args.cases) if args.cases else None,
        )

    if args.database_url:
        return execute(args.database_url)

    with tempfile.TemporaryDirectory(prefix="xssboss-hard-lab-") as temp_dir:
        db_path = (Path(temp_dir) / "acceptance.db").as_posix()
        return execute(f"sqlite:///{db_path}")
