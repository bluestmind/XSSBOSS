...
const ParamDetailPage = () => {
  ...
  if (!param) {
    return (
      <div className="p-6">
        <p className="text-red-300">Parameter not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <Link to={`/endpoints/${param.endpoint_id}`} className="text-brand-300 hover:text-brand-200 text-sm mb-2 inline-block">
          ← Back to Endpoint
        </Link>
        <div className="flex items-center space-x-3">
          <code className="text-lg font-mono text-carbon-100">{param.name}</code>
          <span className={`px-3 py-1 text-sm font-semibold rounded ${getLocationColor(param.location)}`}>
            {param.location}
          </span>
        </div>
        {param.sample_value && (
          <p className="text-sm text-carbon-300 mt-2">
      ...
      <div className="mb-8">
        ...
        {paramContexts.length > 0 ? (
          ...
          <p className="text-carbon-400">No contexts detected yet. Run context detection to find reflections.</p>
        )}
      </div>
