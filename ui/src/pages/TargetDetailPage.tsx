...
const TargetDetailPage = () => {
  ...
  if (!target) {
    return (
      <div className="p-6">
        <p className="text-red-300">Target not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <Link to="/targets" className="text-brand-300 hover:text-brand-200 text-sm mb-2 inline-block">
          ← Back to Targets
        </Link>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-carbon-100">{target.name}</h1>
            <p className="text-carbon-300 mt-1">{target.base_url}</p>
          </div>
        ...
        {target.notes && (
          <p className="text-carbon-300 mt-2">{target.notes}</p>
        )}
        <div className="mt-4 text-sm text-carbon-400">
      ...
      {/* Endpoints */}
      <div className="mb-8">
        <h2 className="text-xl font-semibold text-carbon-100 mb-4">Endpoints</h2>
        <EndpointTable endpoints={endpoints} isLoading={endpointsLoading} />
      </div>

      {/* Experiments */}
      <div className="mb-8">
        <h2 className="text-xl font-semibold text-carbon-100 mb-4">Experiments</h2>
        <ExperimentTable targetId={targetId} />
      </div>

      {/* Findings */}
      <div>
        <h2 className="text-xl font-semibold text-carbon-100 mb-4">Findings</h2>
