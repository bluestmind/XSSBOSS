...
const XssChecklistPage = () => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-carbon-800/70 rounded-xl border border-carbon-700/60 shadow-sm p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-carbon-100 tracking-tight">Ultimate XSS Hunting Checklist</h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold text-carbon-200 bg-carbon-800/60 hover:bg-carbon-700 rounded-lg transition-colors"
            >
              Reset Progress
            </button>
            <button
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
          <div className="space-y-1">
            <div className="flex justify-between text-sm font-semibold text-carbon-200">
              <span>Overall Progress</span>
              <span>{completedCount} / {totalItems} Tasks ({progressPercent}%)</span>
            </div>
            <div className="w-full bg-carbon-800/60 rounded-full h-3">
              <div
          ...
          <div className="flex justify-center md:justify-end gap-2 md:col-span-2">
            <span className="text-xs font-medium text-carbon-500 self-center mr-2">Filter Tasks:</span>
            {(['all', 'pending', 'completed'] as const).map((mode) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg capitalize border transition-all ${
                  filterMode === mode
                    ? 'bg-brand-500/10 text-brand-300 border-brand-500/30 shadow-sm'
                    : 'bg-carbon-800/70 text-carbon-300 border-carbon-700/60 hover:bg-carbon-850/50'
                }`}
              >
      ...
      <div className="space-y-6">
        {sections.map((section) => {
          ...
          return (
            <div key={section.id} className="bg-carbon-800/70 rounded-xl border border-carbon-700/60 shadow-sm overflow-hidden transition-all">
              {/* Section Header */}
              <div
                onClick={() => toggleSection(section.id)}
                className="px-6 py-4 bg-carbon-850/50 border-b border-carbon-700/40 flex items-center justify-between cursor-pointer hover:bg-carbon-800/60/75 transition-colors select-none"
              >
                <div className="flex items-center space-x-3">
                  <span className="text-lg font-bold text-carbon-100">{section.title}</span>
                  <span className="px-2 py-0.5 text-xs font-semibold bg-carbon-700 text-carbon-300 rounded-full">
                    {sectionCompletedCount} / {sectionTotalCount} done
                  </span>
                </div>
                <div className="flex items-center space-x-2 text-carbon-500">
                  <svg
                ...
                <div className="p-6">
                  {section.type === 'list' ? (
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-start gap-4 p-4 rounded-lg border cursor-pointer select-none transition-all ${
                              isDone
                                ? 'bg-carbon-850/50/50 border-carbon-700/60 opacity-70'
                                : 'bg-carbon-800/70 border-carbon-700/60 hover:border-brand-500/40 hover:shadow-sm'
                            }`}
                          >
                            <div className="mt-1 flex items-center justify-center">
                              <input
                                ...
                                checked={isDone}
                                readOnly
                                className="h-4.5 w-4.5 rounded border-carbon-600 text-brand-300 focus:ring-brand-500"
                              />
                            </div>
                            <div className="space-y-1">
                              <h3 className={`font-semibold text-carbon-100 text-sm md:text-base ${isDone ? 'line-through text-carbon-400' : ''}`}>
                                {item.label}
                              </h3>
                              {item.description && (
                                <p className={`text-sm ${isDone ? 'text-carbon-500' : 'text-carbon-300'}`}>
                                  {item.description}
                                </p>
                              )}
                            </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-carbon-700/50 border border-carbon-700/40 rounded-lg">
                          ...
                          <tr>
                            ...
                            {section.headers?.map((header, idx) => (
                              <th
                                key={idx}
                                scope="col"
                                className="px-4 py-3 text-left text-xs font-semibold text-carbon-400 uppercase tracking-wider"
                              >
                                {header}
                              </th>
                            ))}
                          ...
                          {filteredItems.map((item) => {
                            ...
                            return (
                              <tr
                                key={item.id}
                                onClick={() => toggleItem(item.id)}
                                className={`cursor-pointer hover:bg-carbon-850/50/70 transition-colors select-none ${
                                  isDone ? 'bg-carbon-850/50/30 opacity-70' : ''
                                }`}
                              >
                                ...
                                <td className="px-4 py-4 text-sm font-semibold text-carbon-100 max-w-[200px]">
                                  <span className={isDone ? 'line-through text-carbon-500' : ''}>
                                    {item.technique}
                                  </span>
                                </td>
                                <td className="px-4 py-4 text-sm text-carbon-300 max-w-xs md:max-w-md">
                                  <span className={isDone ? 'line-through text-carbon-500' : ''}>
                                    {item.scenario}
                                  </span>
                                </td>
                                <td className="px-4 py-4 text-sm text-brand-300 bg-brand-500/10/20 max-w-xs font-medium">
