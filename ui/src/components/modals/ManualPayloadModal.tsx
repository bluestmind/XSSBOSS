      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-carbon-850 rounded-xl border border-carbon-700 text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
          <div className="bg-carbon-850 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 className="text-lg font-medium text-carbon-100 mb-4">Manual Payload Console</h3>

            <div className="space-y-4">
              {/* Token section */}
              <div>
                <label className="block text-sm font-medium text-carbon-200 mb-2">Oracle Token</label>
                <div className="flex space-x-2">
                  <input
                    ...
                    className="flex-1 rounded-md border-carbon-600 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm"
                  />
                  <button
                    type="button"
                    onClick={generateToken}
                    className="px-4 py-2 bg-carbon-800/60 text-carbon-200 rounded-md hover:bg-carbon-700 text-sm"
                  >
                    Generate
                  </button>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium text-carbon-200">Payload</label>
                  <button
                    type="button"
                    onClick={insertToken}
                ...
                <textarea
                  ...
                  rows={8}
                  className="block w-full rounded-md border-carbon-600 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm font-mono"
              ...
              {/* Live preview */}
              <div>
                <label className="block text-sm font-medium text-carbon-200 mb-2">Live Preview</label>
                <div className="bg-carbon-850/50 rounded-md p-3 border border-carbon-700/60">
              ...
              {executionResult && (
                <div>
                  <label className="block text-sm font-medium text-carbon-200 mb-2">Execution Result</label>
                  {executionResult.error ? (
                    <div className="bg-red-500/10border border-red-500/30 rounded-md p-3">
                      <p className="text-sm text-red-200">{executionResult.error}</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      ...
                      {executionResult.screenshot_path && executionResult.execution_id && (
                        <div>
                          <p className="text-sm font-medium text-carbon-200 mb-1">Screenshot:</p>
                          <img
                            src={`/api/v1/results/executions/${executionResult.execution_id}/screenshot`}
                      ...
                      {executionResult.dom_snapshot && (
                        <div>
                          <p className="text-sm font-medium text-carbon-200 mb-1">DOM Snapshot:</p>
                          <pre className="bg-carbon-850/50 rounded p-3 text-xs overflow-auto max-h-64 border">
