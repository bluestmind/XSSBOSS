  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-carbon-850 rounded-xl border border-carbon-700 text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
          <form onSubmit={handleSubmit}>
            <div className="bg-carbon-850 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <h3 className="text-lg font-medium text-carbon-100 mb-4">Create Experiment</h3>
                    ...
                    <div key={s} className="flex items-center">
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center ${
                          step >= s ? 'bg-brand-600 text-white' : 'bg-carbon-700 text-carbon-300'
                        }`}
                      >
                        {s}
                      </div>
                      {s < 5 && (
                        <div
                          className={`w-12 h-1 ${step > s ? 'bg-brand-600' : 'bg-carbon-700'}`}
                        />
                      )}
                    </div>
                  ))}
              ...
              {step === 1 && (
                <div>
                  <label className="block text-sm font-medium text-carbon-200 mb-2">Select Target *</label>
                  <select
                    required
                    value={formData.target_id || ''}
                    onChange={(e) => setFormData({ ...formData, target_id: parseInt(e.target.value) })}
                    className="block w-full rounded-md border-carbon-600 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm"
                  >
              ...
              {step === 2 && (
                <div>
                  <label className="block text-sm font-medium text-carbon-200 mb-2">Select Endpoint *</label>
                  <select
                    required
              ...
              {step === 3 && (
                <div>
                  <label className="block text-sm font-medium text-carbon-200 mb-2">Select Parameter *</label>
                  <select
                    required
              ...
              {step === 4 && (
                <div>
                  <label className="block text-sm font-medium text-carbon-200 mb-2">Fuzzing Strategy *</label>
                  <div className="space-y-2">
                    {(['smart_adaptive', 'quick_light', 'max_coverage', 'unicode_hunt', 'js_string_specialist', 'csp_aware'] as const).map((strategy) => (
                      <label key={strategy} className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-carbon-850/50">
                        <input
                          type="radio"
                          name="strategy"
                          value={strategy}
                          ...
                          onChange={(e) => setFormData({
                            ...formData,
                            strategy: e.target.value as ExperimentCreate['strategy'],
                          })}
                          className="mr-3"
                        />
                        <div>
                          ...
                          <div className="text-sm text-carbon-400">
                            ...
                            {strategy === 'smart_adaptive' && 'Recommended: compact context-aware coverage with filter-guided bypasses'}
              ...
              {step === 5 && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-carbon-200 mb-2">Experiment Name *</label>
                    <input
                      type="text"
                      required
                      value={formData.name || ''}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-carbon-200 mb-2">Max Test Cases</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-carbon-200 mb-2">Concurrency</label>
                    <input
                      type="number"
                      min="1"
                      max="10"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-carbon-200 mb-2">Timeout (seconds)</label>
                    <input
                      type="number"
                      min="5"
                      max="120"
