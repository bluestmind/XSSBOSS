...
function EndpointsMap({ targetId, monitor }) {
  ...
  // ---------------------------------------------------------------------------
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden text-slate-700 flex flex-col h-[580px]">
      {/* Header bar */}
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <label className="flex items-center gap-1.5 cursor-pointer select-none">
            <input 
              ...
              className="rounded border-slate-300 bg-white text-indigo-600 focus:ring-0 focus:ring-offset-0"
            />
            <span>Params</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer select-none">
          ...
          <button 
            ...
            className="bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 px-2.5 py-1 rounded shadow-sm"
          >
            Reset Layout
          </button>
        </div>
      ...
      <div className="flex-1 flex relative">
        <canvas 
          ...
          onWheel={handleWheel}
          className="flex-1 cursor-grab active:cursor-grabbing bg-slate-50/50"
        />

        {/* Dynamic Nodes Count Stats Overlay */}
        <div className="absolute top-4 left-4 flex flex-col gap-1.5 bg-white/90 backdrop-blur border border-slate-200 rounded p-3 text-xs select-none shadow-sm">
        ...
        <div className="w-[300px] bg-slate-50 border-l border-slate-200 p-5 overflow-y-auto text-xs flex flex-col gap-4">
          <h3 className="font-semibold text-slate-500 uppercase tracking-wider text-[10px] border-b border-slate-200 pb-2">
            Node Inspector
          </h3>
          {selectedNode ? (
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm font-bold capitalize mt-0.5 text-slate-800">{selectedNode.type}</div>
              </div>
              
              {selectedNode.type === 'target' && (
                <div>
                  <div className="text-[10px] text-slate-400 font-semibold uppercase">Root Domain</div>
                  <div className="text-xs font-mono font-bold mt-1 bg-white border border-slate-200 p-2 rounded text-indigo-600 break-all shadow-sm">
                    {selectedNode.label}
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold mt-1 ${
                      selectedNode.method === 'POST' ? 'bg-rose-50 border border-rose-100 text-rose-500' : 'bg-blue-50 border border-blue-100 text-blue-500'
                    }`}>
                      {selectedNode.method}
                    </span>
                  </div>
                  <div>
                    ...
                    <div className="text-xs font-mono mt-1 bg-white border border-slate-200 p-2 rounded text-slate-600 break-all select-all shadow-sm">
                      {selectedNode.label}
                    </div>
                  </div>
                </div>
              ...
              {selectedNode.type === 'param' && (
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="text-sm font-bold text-amber-600 mt-0.5 font-mono">{selectedNode.name}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-slate-400 font-semibold uppercase">Submission Location</div>
                    <span className="inline-block px-2 py-0.5 rounded bg-white border border-slate-200 text-[10px] font-bold text-slate-500 mt-1 capitalize shadow-sm">
              ...
              {selectedNode.type === 'finding' && (
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-mono font-bold mt-1 bg-white border border-slate-200 p-2 rounded text-slate-600 shadow-sm">
                      {selectedNode.param_name || 'Controllable input'}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] text-slate-400 font-semibold uppercase font-bold text-rose-500">Trigger Payload (PoC)</div>
