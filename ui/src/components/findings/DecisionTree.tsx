...
const DecisionTree: React.FC<DecisionTreeProps> = ({ finding }) => {
  ...
  return (
    <div className="bg-carbon-850/50 rounded-lg border border-carbon-700/60 p-6 my-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-md font-semibold text-carbon-100">XSS Checklist Execution Tree</h4>
        <span className="px-2.5 py-0.5 text-xs font-semibold bg-green-500/15 text-green-200 rounded-full">
      ...
      <div className="relative border-l-2 border-brand-500/30 ml-4 space-y-8 pb-4">
        {steps.map((step, index) => (
          <div key={step.id} className="relative pl-8">
            {/* Tree Node Dot */}
            <span className="absolute -left-[9px] top-1 flex h-4 w-4 items-center justify-center rounded-full bg-brand-500 ring-4 ring-carbon-900">
              <span className="h-1.5 w-1.5 rounded-full bg-carbon-800/70" />
            </span>

            {/* Content Container */}
            <div className="bg-carbon-800/70 rounded-lg border border-carbon-700/60 p-4 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-semibold text-brand-300 uppercase tracking-wider">{step.title}</span>
                <span className="px-2 py-0.5 text-[10px] font-medium bg-green-500/10text-green-300 rounded border border-green-500/30">
                  PASSED
                </span>
              </div>
              <h5 className="text-sm font-semibold text-carbon-100 mb-1">{step.detail}</h5>
              <p className="text-xs text-carbon-400 leading-relaxed mb-3">{step.description}</p>

              {/* Decisions metadata sub-table */}
              <div className="bg-carbon-850/50 rounded border border-carbon-700/50 p-2.5">
                <span className="text-[10px] font-bold text-carbon-500 uppercase block mb-1.5">Checks & Decisions Made</span>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                  {step.decisions.map((dec, i) => (
                    <div key={i} className="flex flex-col">
                      <span className="text-[10px] text-carbon-500 font-medium">{dec.key}</span>
                      <span className="font-mono text-carbon-100 break-all truncate" title={dec.value}>
                        {dec.value}
                      </span>
                    </div>
                  ))}
