...
class FilterProfiler:
    ...
    PROBE_PAYLOADS = [
        ...
        {
            'name': 'javascript_tagged_template',
            'payload': 'javascript:alert`1`',
            'purpose': 'Test javascript: protocol with tagged template literal without parentheses'
        },
        {
            'name': 'svg_onload',
    ...
    def send_probe(
        ...
        try:
            ...
            # If response is blocked by WAF/bot-defense on GET or query/path, fallback to headless browser probe
            if (
                response.status_code in (401, 403, 429, 503)
                and (method.upper() == 'GET' or param_location in ['query', 'path'])
            ):
                try:
                    ...
                    try:
                        ...
                        if "access denied" not in (b_driver.title or "").lower():
                            class MockBrowserResponse:
                                ...
                                text = b_text
                                headers = {}
                            response = MockBrowserResponse()
                        else:
                            waf_name = FilterProfiler._detect_waf(response.status_code, response.text, dict(response.headers)) or "WAF"
                            return {
    ...
    def _detect_waf(
        ...
        """
        waf_indicators = {
            'aws_waf': ['awswaf', 'x-amzn-waf-action', 'x-amz-cf-id', 'cloudfront', 'amazons3'],
            'cloudflare': ['cloudflare', 'cf-ray'],
            'incapsula': ['incapsula', 'x-iinfo', 'imperva'],
    ...
    def profile_endpoint_param(
        ...
        for probe in FilterProfiler.PROBE_PAYLOADS:
            ...
            # If site-wide WAF is active, don't spam 25 probes
            if result.get('waf_detected'):
                waf_blocks = sum(1 for r in probe_results if r.get('waf_detected'))
                if waf_blocks >= 2:
                    logger.info(f"WAF actively blocking param {param_name}; short-circuiting remaining probes.")
                    break
                # Extract blocked tokens
                tokens = FilterProfiler._extract_tokens(payload)
                blocked_tokens.extend(tokens)
            elif result.get('reflected') and not result.get('escaped'):
                # Payload reflected without escaping - tokens allowed
                tokens = FilterProfiler._extract_tokens(payload)
                allowed_tokens.extend(tokens)
            
            # Track normalization
            if result.get('normalized') and result.get('normalization_behavior'):
                behaviors = result.get('normalization_behavior')
        ...
        try:
            ...
            def remote_transform(probe_str: str) -> str:
                res = FilterProfiler._send_probe(
                    method=method, url=url, param_name=param_name,
                    param_value=probe_str, param_location=param_location,
                    headers=headers, cookies=cookies, base_body=base_body, base_json=base_json, timeout=timeout
                )
                return res.get('raw_reflection', '') or ''
            
            s_prof = AutomataLearner.learn_sanitizer(remote_transform)
            if s_prof and s_prof.library_name != "Custom/Unknown Sanitizer":
                sanitizer_detected = f"{s_prof.library_name} ({s_prof.version_range})"
                verified_cve_bypasses = s_prof.verified_cve_bypasses
        except Exception as auto_err:
            logger.debug(f"AutomataLearner in FilterProfiler failed: {auto_err}")
        ...
        return {
            ...
            'sanitizer_detected': sanitizer_detected,
            'verified_cve_bypasses': verified_cve_bypasses,
            'csp_rules': csp_rules,
    ...
    def _extract_tokens(payload: str) -> List[str]:
        ...
        # Extract character-level probe targets (e.g. xss(xss -> '(')
        if len(payload) == 7 and payload.startswith('xss') and payload.endswith('xss'):
            tokens.append(payload[3])

        # Extract special characters tested by probes
        special_chars = ['(', ')', '[', ']', '{', '}', "'", '"', '`', '<', '>', '/', '\\', ';']
        for sc in special_chars:
            if sc in payload and sc not in tokens:
                tokens.append(sc)
