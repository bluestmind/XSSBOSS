...
def _encoded_token_pairs(old_token: str, new_token: str):
    """Yield equivalent token encodings without decoding the whole payload.

    Payload generators intentionally obfuscate the oracle token.  Replacing only
    its literal form leaves a stale token inside HTML entities or JavaScript
    escapes, which makes an otherwise valid execution impossible to correlate.
    """
    encoders = (
        lambda value: "".join(f"&#{ord(char)};" for char in value),
        lambda value: "".join(f"&#x{ord(char):x};" for char in value),
        lambda value: "".join(f"&#X{ord(char):X};" for char in value),
        lambda value: "".join(f"\\x{ord(char):02x}" for char in value),
        lambda value: "".join(f"\\x{ord(char):02X}" for char in value),
        lambda value: "".join(f"\\u{ord(char):04x}" for char in value),
        lambda value: "".join(f"\\u{ord(char):04X}" for char in value),
        lambda value: "".join(f"%{ord(char):02x}" for char in value),
        lambda value: "".join(f"%{ord(char):02X}" for char in value),
    )
    for encode in encoders:
        yield encode(old_token), encode(new_token)
...
class Tokenizer:
    ...
    def replace_token(payload: str, old_token: str, new_token: str) -> str:
        ...
        # Replace obfuscated forms before the literal token. This is important
        # when a generator encoded an entire JavaScript expression as entities.
        for encoded_old, encoded_new in _encoded_token_pairs(old_token, new_token):
            payload = payload.replace(encoded_old, encoded_new)
    ...
    def replace_token_with_placeholders(payload: str, token: str) -> str:
        ...
        # 4. HTML/JS/percent-encoded token forms
        placeholder_kinds = (
            "HTML_DEC", "HTML_HEX", "HTML_HEX_UPPER", "JS_HEX",
            "JS_HEX_UPPER", "JS_UNICODE", "JS_UNICODE_UPPER",
            "PERCENT", "PERCENT_UPPER",
        )
        for (encoded_token, _), kind in zip(_encoded_token_pairs(token, token), placeholder_kinds):
            payload = payload.replace(encoded_token, f"{{{{TOKEN_{kind}}}}}")
    ...
    def replace_placeholders_with_token(payload: str, token: str) -> str:
        ...
        encoded_values = [new for _, new in _encoded_token_pairs(token, token)]
        ...
        for kind, encoded_value in zip(placeholder_kinds, encoded_values):
            payload = payload.replace(f"{{{{TOKEN_{kind}}}}}", encoded_value)
        payload = payload.replace("{{TOKEN}}", token)
