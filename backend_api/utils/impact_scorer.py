...
class ImpactScorer:
    ...
    def score_components(
        ...
        score = min(score, 100)
        base_severity = ImpactScorer._severity_for_score(score)

        # Composed exploitability evaluation (SameSite, COOP, CSP Trusted Types, HttpOnly)
        from backend_api.services.exploitability_composer import ExploitabilityComposer
        verdict = ExploitabilityComposer.compose(
            base_severity=base_severity,
            method=endpoint.method or "GET",
            headers=verification.get("headers"),
            cookies=verification.get("cookies"),
            is_stored=bool(verification.get("stored")),
        )

        final_severity = verdict.adjusted_severity
        tags.extend(verdict.tags)
        rationale.extend(verdict.rationales)

        return {
            "score": score,
            "severity": final_severity,
            "severity_value": final_severity.value,
            "base_severity": base_severity.value,
            "exploitability_verdict": verdict.to_dict(),
            "tags": list(dict.fromkeys(tags)),
            "rationale": rationale,
