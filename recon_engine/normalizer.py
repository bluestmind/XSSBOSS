...
class RequestNormalizer:
    ...
    def extract_parameters(
        ...
        for key, value in query_dict.items():
            ...
            # Deep parameter unpacking: Check if query value contains nested JSON or Base64 JSON
            if value and isinstance(value, str):
                ...
                if (val_clean.startswith('{') and val_clean.endswith('}')) or (val_clean.startswith('[') and val_clean.endswith(']')):
                    try:
                        parsed_nested = json.loads(val_clean)
                    except Exception:
                        pass

                # 2. Base64 or URL-Safe Base64 encoded JSON
                if not parsed_nested and len(val_clean) >= 8 and ' ' not in val_clean:
                    try:
                        padded = val_clean + '=' * (-len(val_clean) % 4)
                        decoded_bytes = base64.b64decode(padded.replace('-', '+').replace('_', '/'), validate=False)
                        decoded_text = decoded_bytes.decode('utf-8', errors='ignore').strip()
                        if decoded_text.startswith('{') and decoded_text.endswith('}'):
                            parsed_nested = json.loads(decoded_text)
                    except Exception:
                        pass

                if isinstance(parsed_nested, dict):
                    for sub_k, sub_v in parsed_nested.items():
                        params['query'].append({
                            'name': f"{key}.{sub_k}",
                            'location': ParamLocation.QUERY.value,
                            'sample_value': str(sub_v) if sub_v is not None else None
                        })
