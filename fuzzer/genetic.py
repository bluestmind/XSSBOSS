...
class FitnessScorer:
    ...
    def calculate_fitness(test_case: TestCase, execution: Execution) -> float:
        ...
        token = test_case.token or ""

        # 2. Check for dynamic sink hits or source reads (from instrumentation callbacks)
        sink_type = logs.get('sink')
        if sink_type:
            # Require token co-occurrence in sink telemetry to avoid rewarding unrelated background activity
            sink_data_str = str(logs.get('data', '')) + " " + str(logs.get('sample', '')) + " " + str(logs.get('blocked_uri', ''))
            token_in_sink = bool(token and (token in sink_data_str or token in str(logs)))

            if sink_type == 'securitypolicyviolation':
                if token_in_sink:
                    score = max(score, 90.0)  # High fitness: reached execution point with our payload but blocked by CSP
            elif sink_type.startswith('DOMSourceRead:') or sink_type == 'postMessage.source':
                if token_in_sink:
                    score += 40.0  # Taint source successfully read with our token
            else:
                if token_in_sink:
                    score += 50.0  # Hit a dangerous sink (e.g., innerHTML) with our token
                    # If it hit innerHTML but is NOT reflected in DOM, it was likely sanitized by DOMPurify
                    if execution.dom_snapshot and token not in execution.dom_snapshot:
                        score += 25.0  # Additional fitness weight to trigger sanitizer evasion path!

        # 3. Check for syntax errors (breakout indicator with token co-occurrence)
        ...
        for err in errors:
            ...
            if token and token in err:
                score += 30.0
                break
            elif token and any(kw in err_lower for kw in ["syntax", "unexpected token", "invalid or unexpected", "referenceerror"]) and token in str(logs):
                score += 20.0
                break
        ...
        if execution.dom_snapshot:
            ...
            token_count = execution.dom_snapshot.count(token) if token else 0
            if token_count > 0:
    ...
    def evolve_next_generation(
        ...
        if best_fitness < 100.0:
            try:
                ...
                telemetry = parse_execution_logs(best_exec.logs) if best_exec else {}
        ...
        # Local Grammar Fuzzer: Seed locally verified fixpoint / mXSS candidates into new generation
        try:
            from fuzzer.local_grammar_fuzzer import LocalGrammarFuzzer
            local_cand = LocalGrammarFuzzer.evolve_grammar_population(
                token=token,
                generations=1,
                population_size=5,
                blocked_chars=set((filter_profile or {}).get("blocked_tokens", []))
            )
            for lc in local_cand:
                if len(next_gen_payloads) < population_size:
                    next_gen_payloads.append(lc)
        except Exception:
            pass
